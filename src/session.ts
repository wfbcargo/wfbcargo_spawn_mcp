...
8. After meaningful pushes, spawn_savi with what changed — and use it to DELEGATE. Savi reads the creator's studio chat and can take a task on, fanning it out across its own sub-agents, so hand over whole separable slices with the task argument rather than building them yourself. State it broadly and let Savi split it; pass keepOff so its sub-agents route around what you are still in. Nothing comes back on this channel, which has one consequence worth holding: never sequence your own work behind a handoff. Read the result as head moving past your own last push, then spawn_latest and screenshot it like any other change. The tool description has the rest.
...
- Coordinate with Savi via spawn_savi, and count Savi as capacity rather than a bystander: a broad task handed over there comes back built by sub-agents you never had to key, check out, or supervise. Partition against it the way you would against a teammate — claim your area, pass those same patterns as keepOff — and treat the pushes that result like any other agent's on the version rail. They are literally indistinguishable: no endpoint reports an author, so an unexplained head bump is Savi, the creator, or a teammate, and only the team ledger can tell you which.
