...
├── audit-tools.ts    LOCAL audit: no network (sweep.ts, harness.ts, builtins.ts)
...
`audit-tools.ts` → `sweep.ts`; `asset-tools.ts` → `assets.ts`. Keep that split — the
...
## The local audit (`audit-tools.ts` → `sweep.ts`, `harness.ts`, `builtins.ts`)
...
fact is the entire basis for the local audit: `harness.ts` loads game scripts,
...
Per-game assertions live in the **game project** (`audit/math.json`), not in this
server: this server owns the runner, the game owns the tests.
