...
1. **A Spawn account** — [spawn.co](https://www.spawn.co).
2. **An AI coding assistant that supports MCP** — Cursor, Claude Code, Claude Desktop, or another MCP-compatible tool.
3. **Node.js version 18 or newer** — [nodejs.org](https://nodejs.org). To check if you already have it, open a terminal and run `node --version`.
...
Then, if you want your assistant to be able to *see* your game (recommended — it's most of the value):
...
- `wfbcargo_spawn_mcp` — the tool you just installed. You won't edit anything in here.
- `my-first-game` — your actual game. Make this now, anywhere you like.
...
Your assistant will trade that short-lived key for a long-lived one and save it in a `.env` file inside your game folder. From then on it stays connected — you won't need to do this again.
...
| "Bootstrap failed" / "expired" | The `sbk_` key expired — they last ~5 minutes. Get a fresh one and retry immediately. |
...
- [README](README.md) — the full tool list and the technical reference.
